...
class TownDashboardPageProcessor extends PageProcessorCredentialSupport {
  ...
  async #internalProcess(credential: Credential) {
    const configId =
      TownDashboardLayoutManager.loadDashboardLayoutConfigId(credential);
    const layout = LAYOUT_MANAGER.getLayout(configId);
    const parser = new TownDashboardPageParser(
      credential,
...
function doRenderMenu(credential: Credential, page: TownDashboardPage) {
  ...
  if (bsId >= 0) {
    ...
    $("#menuTable")
      ...
      .each((idx, th) => {
        const extensionId =
          SetupLoader.getTownDashboardExtensionShortcutButton();
        $(th).html("");
        if (extensionId > 0) {
