...
class MapDashboardPageProcessor extends PageProcessorCredentialSupport {
  async doProcess(
    ...
    $("#systemAnnouncement").after(
      $(
        "<div id='version' style='color:navy;font-weight:bold;text-align:center;width:100%'></div>"
      )
    );
    // @ts-ignore
    ...
    if (roleTask === null || roleTask === "") {
      // 如果有必要的话绘制城堡
      new CastleInformation().load(page.role!.name!).then((castle) => {
    ...
    $("td:contains('经验值')")
      ...
      .each(function (_idx, th) {
        ...
        const experience = parseInt(
          StringUtils.substringBefore(expText, " EX")
        );
        if (experience >= 14900) {
          ...
          $(th).html(
            "<span title='" + expText + "'>" + progressBar + "</span>"
          );
        }
      });
  }
  ...
  #doTravelToLocation(credential: Credential, location: Coordinate) {
    MessageBoard.publishMessage(
      "目的地坐标：<span style='color:greenyellow'>" +
        location.asText() +
        "</span>"
    );

    const plan = TravelPlanBuilder.initializeTravelPlan(
