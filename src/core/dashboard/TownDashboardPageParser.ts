...
function _parseMobilization(page: TownDashboardPage, table: JQuery) {
  $(table)
    ...
    .each((idx, font) => {
      ...
      const b2 = SetupLoader.isQiHanTitleEnabled()
        ? RankTitleLoader.transformTitle(ss[1])
        : ss[1];
      const b3 = ss[2];
...
function _parseBattleMenu(
  page: TownDashboardPage,
  table: JQuery,
...
function _parseRoleStatus(
  ...
  $(div)
    ...
    .each((_i, td) => {
      ...
      idx = s.indexOf("智力：");
      s = s.substring(idx);
      page.obtainRole.specialAttack = _.parseInt(
        s.substring(3, s.indexOf(" "))
      );
      idx = s.indexOf("精神力：");
      s = s.substring(idx);
      page.obtainRole.specialDefense = _.parseInt(
  ...
  if (
    page.obtainRole.level === 150 &&
    !SetupLoader.isCareerTransferEntranceDisabled(credential.id)
  ) {
