  ...
  static removeGoogleAnalyticsScript() {
    ...
    $("script")
      .filter(function () {
        ...
        return (
          src !== undefined && (src as string).includes("google-analytics")
        );
      })
  ...
  static findFirstRoleImageHtml() {
    ...
    $("img").each(function (_idx, img) {
      if (roleImage === "") {
        ...
        if (
          src !== undefined &&
          src.startsWith(Constants.POCKET_DOMAIN + "/image/head/")
        ) {
          // 发现了用户头像
          roleImage = src;
        }
  ...
  static fixCurrentPageBrokenImages() {
    ...
    $("img")
      .filter(function () {
        ...
        return (
          src.startsWith("http://pocketrose.21sun.net:81/pocketrose") ||
      ...
      .each(function (_idx, img) {
        ...
        if (src.includes("http://pocketrose.21sun.net:81/pocketrose")) {
          s = StringUtils.substringAfter(
            s,
            "http://pocketrose.21sun.net:81/pocketrose"
          );
        } else if (src.includes("http://pocketrose.21sun.net/pocketrose")) {
          s = StringUtils.substringAfter(
            s,
            "http://pocketrose.21sun.net/pocketrose"
          );
        }
